...
void finSAR_editOverlayFactory::DrawEBLLineInViewPort(PlugIn_ViewPort *BBox) {
...
void finSAR_editOverlayFactory::DrawIndexTargets(PlugIn_ViewPort *BBox) {
  ...
  for (std::vector<IndexTarget>::iterator it = m_dlg.i_vector.begin();
    ...
    if (!image.GetOrFindMaskColour(&mr, &mg, &mb) && !a)
      wxMessageBox("trying to use mask to draw a bitmap without alpha or mask\n");
...
void finSAR_editOverlayFactory::DrawBearingLineInViewPort(
...
void finSAR_editOverlayFactory::DrawGLLine(double x1, double y1, double x2,
...
void finSAR_editOverlayFactory::DrawMessageWindow(wxString msg, int x, int y,
                                                  wxFont *mfont) {
  if (msg.empty()) return;
...
wxImage &finSAR_editOverlayFactory::DrawGLText(double value, int precision) {
  wxString labels;
...
wxImage &finSAR_editOverlayFactory::DrawGLDisk(double value, int precision) {
  wxString labels;
...
wxImage &finSAR_editOverlayFactory::DrawGLTextString(wxString myText) {
  wxString labels;
  labels = myText;
...
void finSAR_editOverlayFactory::DrawOLBitmap(const wxBitmap &bitmap, wxCoord x,
...
void finSAR_editOverlayFactory::DrawGLLabels(finSAR_editOverlayFactory *pof,
                                             ...
                                             wxBitmap &imageLabel, double myLat,
  ...
  if (dc) {
    ...
    dc->DrawBitmap(imageLabel, xd, yd, false);
  } else { /* opengl */

    wxImage il = imageLabel.ConvertToImage();

    int w = il.GetWidth(), h = il.GetHeight();

    unsigned char *d = il.GetData();
    unsigned char *a = il.GetAlpha();

    unsigned char mr, mg, mb;
    if (!il.GetOrFindMaskColour(&mr, &mg, &mb) && !a)
      wxMessageBox(
          _T(
...
void finSAR_editOverlayFactory::drawGLPolygons(finSAR_editOverlayFactory *pof,
